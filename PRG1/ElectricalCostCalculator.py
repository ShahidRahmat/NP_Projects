...
mthly_cost = str(input("Enter the electrical cost($) for 3 months separated by semicolon: "))
...
def calculate_average(mthly_cost):
    ...
    print(mthly_cost)
    total = float(mthly_cost[0]) + float(mthly_cost[1]) + float(mthly_cost[2]) #Compute the total electrical costs over 3 months
    average =  total/3 #Compute the average cost
